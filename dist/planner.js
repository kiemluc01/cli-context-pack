...
export const codexRoot = (skill) => `.codex/skills/${skill}`;
...
export function buildPlan(input) {
    ...
    for (const skill of input.skills) {
        ...
        if (input.config.targets.includes("claude"))
            planLinkedTarget(skill, locked, linkMode, input, actions, claudeRoot);
        if (input.config.targets.includes("codex"))
            planLinkedTarget(skill, locked, linkMode, input, actions, codexRoot);
        if (input.config.targets.includes("copilot"))
            planCopilotTarget(skill, input, actions);
...
function planLinkedTarget(skill, locked, mode, input, actions, rootFor) {
...
function planCopilotTarget(skill, input, actions) {
    const rel = `.github/instructions/${skill.name}.instructions.md`;
    const source = skill.files.find((f) => f.rel === "SKILL.md");
    if (!source)
        return;
    const marker = "<!-- managed-by: ctxpack; canonical source: .agent/skills/";
    const desired = `${marker}${skill.name}/SKILL.md -->\n\n${source.content.toString()}`;
    const existing = readTextOrNull(toAbs(input.root, rel));
    if (existing === desired)
        return;
    if (existing !== null && !existing.startsWith(marker)) {
        actions.push({ kind: "conflict", path: rel, code: "CONFLICT", detail: "Copilot instruction file exists and is not managed by ctxpack.", fix: `Move ${rel} aside, then run "ctxpack install copilot".` });
        return;
    }
...
function planClaudeMd(input, actions) {
    ...
    const lines = [
        ...
        "Every question or confirmation you need from the user goes through the AskUserQuestion tool: gate",
        "items, follow-ups, HIGH_RISK go-aheads, conventions taken from context/ that you want confirmed,",
        "and approval to edit outside the change scope. One question per item, selectable options with the",
        "recommended one first; at most 4 questions per call, the rest in a second call right after. Never",
        "type a question list into chat or ask the user to type an answer (\"reply 1b 2c\", \"trả lời ngắn\").",
        "A \"Taken from existing context\" line only states decisions; confirming them is a tool question.",
