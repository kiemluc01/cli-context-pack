...
import { lstatOrNull, readTextOrNull, toAbs } from "./fsutil.js";
...
export function install(ctx, platform) {
    const loc = locateProject(ctx.cwd);
...
export function status(ctx) {
    ...
    const platforms = ["claude", "codex", "copilot"].map((name) => ({
        name,
        configured: config.targets.includes(name),
        installed: config.targets.includes(name) && platformInstalled(loc.root, name, config.skills),
    }));
    if (ctx.json) {
        ctx.io.out(JSON.stringify({ command: "status", root: loc.root, inSync, skills, platforms, renamed, linkMode: p.linkMode, pendingChanges: p.changes, conflicts: p.conflicts, modified }, null, 2));
    }
    else {
        ...
        for (const platform of platforms)
            ctx.io.out(`  ${platform.installed ? "✓" : "○"} ${platform.name}${platform.installed ? "" : " — not installed"}`);
...
function platformInstalled(root, platform, skills) {
    return skills.every((skill) => {
        if (platform === "copilot")
            return readTextOrNull(toAbs(root, `.github/instructions/${skill}.instructions.md`)) !== null;
        const rel = platform === "claude" ? `.claude/skills/${skill}` : `.codex/skills/${skill}`;
        return lstatOrNull(toAbs(root, rel)) !== null;
    });
}
export function doctor(ctx) {
    ...
    if (config) {
        try {
            ...
            for (const platform of ["claude", "codex", "copilot"]) {
                if (!config.targets.includes(platform))
                    add(`platform:${platform}`, "warn", `${platform} adapter not installed`, `Run "ctxpack install ${platform}".`);
                else if (!platformInstalled(loc.root, platform, config.skills))
                    add(`platform:${platform}`, "fail", `${platform} adapter is incomplete`, `Run "ctxpack install ${platform}".`);
                else
                    add(`platform:${platform}`, "ok", `${platform} adapter installed`);
            }
            add("benchmarks", "ok", "benchmark suite is package-managed", 'Run "npm run benchmark:self-test" for local checks.');
