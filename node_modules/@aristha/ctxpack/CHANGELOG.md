...
- **Changed:** the Context Gate opens through the agent's interactive question tool (`AskUserQuestion` in Claude Code) instead of a question list typed into chat. Each checklist item becomes one question with 2-4 selectable options, the recommended option first and labelled `(Recommended)`, HIGH_RISK questions prefixed `[HIGH_RISK]`; at most 4 questions per call, the rest in a second call right after. Follow-ups (`1.1`, `3.1`) use the same tool. The gate no longer ends with a typed-reply instruction ("reply 1b 2c"). The rule covers every question or confirmation, not only gate items: HIGH_RISK go-aheads, confirming decisions taken from `context/` or conventions, and later approvals such as editing outside CHANGE-SCOPE. A "Taken from existing context" line only states decisions and never asks for a typed reply. The text format stays as a fallback for agents without such a tool. `SKILL.md`, `workflows/context-gate.md`, `workflows/implement-feature.md` and the `CLAUDE.md` managed block all say so.
