...
It is not a skill installer, prompt manager, `.claude` config manager, vector database, chatbot, MCP wrapper or agent marketplace. If a task drifts there, stop and confirm. Platform installation is handled by the CLI adapter layer; this skill remains the canonical governance rule set.

## Execution mode

State the selected mode in the execution context before implementation:

```
MODE: lite | full | ultra
```

- **lite**: implement normally and notice an obvious simpler existing solution.
- **full**: apply the complete Solution Decision Ladder below and record the chosen path.
- **ultra**: actively ask whether something can be deleted, reused or simplified before adding code.

`ultra` is advisory about design effort. It never removes correctness, security, authorization, validation, error handling, data-loss protection, observability, accessibility, business rules or required tests.

## Solution Decision Ladder

After understanding the task and tracing the controlling code path, evaluate in order:

1. Does this change need to exist?
2. Does the codebase already provide it?
3. Can an existing project utility or component be reused?
4. Can the standard library solve it?
5. Can an already-installed dependency solve it?
6. Can the requirement use a simpler implementation?
7. What is the smallest correct implementation?

Record the first viable option. The ladder is a minimalism check, not permission to remove safety controls or explicit requirements. Load `skills/minimalism-review/SKILL.md` for a separate advisory review when the task requests design scrutiny.
...
→ TASK CLASSIFICATION → SKILL ROUTER → CONTEXT SELECTOR → SOLUTION LADDER
...
Do not load every skill or context file. Classify the request first (`implement`, `bug-fix`, `refactor`, `validation`, `security`, `performance`, `release`, or `governance`), then select only the skills, references and validation required by that class. Prefer the narrowest path that can satisfy the acceptance criteria. Record:
...
Unselected skills and context are intentionally skipped. Repeated rules belong in this canonical skill or a referenced skill, not in every task prompt.
...
Write the stopping rule before editing:

```
STOP WHEN:
- acceptance criteria pass
- required tests and validation pass
- the declared scope is satisfied
- no required validation remains
- no unresolved blocker exists
```

Do not continue refactoring because additional improvements are possible, including in `full` or `ultra` mode.
